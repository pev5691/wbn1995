...
class AccountApp extends require("./dapp")
{
    ...
    Start(bClean)
    {
        ...
        this.DBStateWriteInner({Num:10, PubKey:GetArrFromHex(ARR_PUB_KEY[2]), Value:{BlockNum:1, SumCOIN:0}, Name:" First Investor"})
        this.DBStateWriteInner({Num:11, PubKey:GetArrFromHex(ARR_PUB_KEY[3]), Value:{BlockNum:1, SumCOIN:0}, Name:" VIP Investor"})
        this.DBStateWriteInner({Num:12, PubKey:GetArrFromHex(ARR_PUB_KEY[4]), Value:{BlockNum:1, SumCOIN:0}, Name:" Bounties P&R Fond"})
        this.DBStateWriteInner({Num:13, PubKey:GetArrFromHex(ARR_PUB_KEY[5]), Value:{BlockNum:1, SumCOIN:0}, Name:" Teamlead Fond"})
        this.DBStateWriteInner({Num:14, PubKey:GetArrFromHex(ARR_PUB_KEY[6]), Value:{BlockNum:1, SumCOIN:0}, Name:" WBN industrial Fond"})
        this.DBStateWriteInner({Num:15, PubKey:GetArrFromHex(ARR_PUB_KEY[7]), Value:{BlockNum:1, SumCOIN:0}, Name:" Social department Fond"})
    ...
    DoCoinBaseTR(Block)
    {
        ...
        if(Data && Data.Currency === 0 && Data.BlockNumCreate < Block.BlockNum)
        {
            ...
            if(!ISZERO(CoinSum))
            {
                ...
                {
                    OperationNum++
                    this.SendMoneyTR(Block, 0, 8, CoinDevelop, Block.BlockNum, 0xFFFF, "", "Founder account", 1, 0, OperationNum)
                }
                {
                    OperationNum++
                ...
                {
                    OperationNum++
                    this.SendMoneyTR(Block, 0, 10, CoinDevelop, Block.BlockNum, 0xFFFF, "", "Incoming P&R fond", 1, 0, OperationNum)
                }
                {
                    OperationNum++
                    this.SendMoneyTR(Block, 0, 11, CoinDevelop, Block.BlockNum, 0xFFFF, "", "Fond no 1", 1, 0, OperationNum)
                }
                {
                    OperationNum++
                    this.SendMoneyTR(Block, 0, 12, CoinDevelop, Block.BlockNum, 0xFFFF, "", "Fond № 2", 1, 0, OperationNum)
                }
                {
                    OperationNum++
                    this.SendMoneyTR(Block, 0, 13, CoinDevelop, Block.BlockNum, 0xFFFF, "", "Fond bounty", 1, 0, OperationNum)
                }
                {
                    OperationNum++
